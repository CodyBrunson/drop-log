...
class DropLog extends Plugin {
    ...
    private cleanupHealthTracker(entityId: number): void {
        try {
            ...
            if (tracker) {
                const npc =
                    this.gameHooks?.EntityManager?.Instance?.NPCs?.get(
                        entityId
                    );
                if (
                    npc &&
    ...
    private checkForDrops(): void {
        try {
            const groundItems =
                this.gameHooks?.GroundItemManager?.Instance
                    ?._groundItems;
        ...
        if (!this.filteredData || this.filteredData.length === 0) {
            this.virtualScrollContent.innerHTML = `
                <div class="drop-log-empty">
                    ${
                (!this.data.dropData || Object.keys(this.data.dropData).length === 0)
    ...
    private ensureObjectStore(): boolean {
        try {
